...
- **abstract module + 値レベル選択**: `tasks/vcs/iface.pkl` のような抽象モジュールを `tasks/vcs/jj.pkl` / `tasks/vcs/git.pkl` が `amends` で実装。`tasks/vcs/auto.pkl` が `.jj` の存在で実装を切り替えるエントリ
...
| `vcs/jj.pkl`    | jj 実装 (amends iface) |
| `vcs/git.pkl`   | git 実装 (amends iface) |
