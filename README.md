...
- Public task names (`vcs:commit`, `docs:check-translations`, `semver:compare`, `migrate:check-pkf-tasks-current`, etc.)
...
Pin to `pkf-tasks@2` for major-only floating, or to an exact version (e.g. `pkf-tasks@2.0.0`) for full reproducibility. The `migrate:check-*` gates keep the pin up to date regardless.
